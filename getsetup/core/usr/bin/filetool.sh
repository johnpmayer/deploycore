...
failed(){
echo "${RED}WARNING: Selected operation has failed. Proceed with extreme caution.${NORMAL}"
echo -n "Press Enter to continue:" ; read ans
clean_up 98
}
...
EOD
     ...
     if [ -f /etc/sysconfig/bfe ]; then
        echo
        echo "${RED}Warning PROTECT boot code used and encrypted backup file not found!"
        echo "${YELLOW}Proceeding with normal restore operations."
        echo "${GREEN}Encryption will occur upon next backup or shutdown.${NORMAL}"
        echo -n "Press enter to continue:" ; read ans
     fi
  fi
# End bfe  
